...
- `GET /api/assets` and `GET /api/assets/:id` for Ninja hardware warranties plus demo software assets
...
- Churn scoring is deterministic. Account age, receivables, payment timing, SLA, and case counts use the five-minute normalized ConnectWise cache when available; invoice aging applies a demo Net 30 effective due date regardless of the CW billing-term label. Missing fields and repeat-case count use labeled neutral demo defaults. Unmapped tenants retain their seeded assessments.
- OpenAI writes only the assessment narrative and suggested actions. It receives the deterministic score and business metrics without live/demo provenance labels. Narratives are persisted by versioned deterministic input fingerprint, reused while inputs and the prompt version are unchanged, and may be explicitly regenerated by an administrator.
...
- NinjaOne uses the OAuth client-credentials flow with the read-only `monitoring` scope. Device scoping uses the documented URL-encoded `df` expression, for example `org = 456`. Hardware warranty dates come from `references.warranty` when populated, with the organization-scoped custom fields `warrantyStartDate` and `warrantyEndDate` used as the operational fallback; seeded hardware is replaced while seeded software remains available.
